...
(function( $ ){
  ...
  $.fn.autocue = function( options ) {
    ...
    $(this).append("<div id='autocueControls'><div id='fastslow'><div id='slower' title='slower'>-</div><div id='faster' title='Faster'>+</div></div>"+controls+"</div>");
    ...
    $("#autocueControls").css({'width':settings.controlSize});  // Define the control sizeo
    $("#autocueControls").css({'opacity':settings.controlOpacity}); // Set the control opacity
    $("#autocueControls").css('borderRadius',settings.controlRadius); // Set the radius of the controls
    $("#fastslow").css('borderRadius',settings.controlRadius); // Set the radius of the fast/slow
    $("#autocueControls").css(settings.controlLocation);  // Set the location of the controls
    $("#autocueControls").css({'backgroundColor':settings.controlBGColor}); // Set the background color of the controls
    ...
    $("#autocueControls").children().bind('selectstart',function(){return false;}); // Stop text selection pt2
  ...
  // her is where I have a problem..
    ...
    if (settings.hoverRequired == 'true'){
      $('#autocueControls').css({opacity:'.2'});
      $('#autocueControls').hover(
        function(){
          $('#autocueControls').animate({opacity:'.8'})
        },
        function(){
          $('#autocueControls').css({opacity:'.1'})
        }
      )
    }
  };
